...
def get_model():
    ...
    for i in range(Config.Unet_blocks):
        if i == 0:
            generator.add(
                Conv2D(filters=Config.imShape[2] * Config.pool_kernel ** (2 * (i + 1)), kernel_size=Config.conv_kernel,
                       padding="same",
    ...
    generator.add(Conv2D(filters=Config.imShape[2], kernel_size=Config.conv_kernel, padding="same", activation="tanh"))
